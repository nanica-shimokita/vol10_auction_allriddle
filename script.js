...
function showIndexPage() {
    ...
    riddles.forEach((riddle, index) => {
        ...
        if (riddle.multi) {
            ...
            isCompleted = solvedCount === totalAnswers;
        }
        ...
        if (isInteractive) {
            ...
            itemClass += ' bg-gray-900/40 cursor-default opacity-60'; // 専用クラスでグレーアウトを適用
    ...
    if (currentRiddle.multi) {
        ...
        if (correctAnswers.includes(userAnswer)) {
            ...
            if (newSolvedAnswers.length === totalAnswers) {
                ...
                document.getElementById('submit-button').disabled = true;
                answerInput.placeholder = "（すべての答えを解答済みです）";
                saveCompletion(currentLevelIndex); // メイン進捗にも完了をマーク
            } else {
                setTimeout(() => { feedbackMessage.textContent = '', 1500 });
            }
        } else {
        ...
        if (userAnswer === correctAnswer) {
            // 正解の場合
            saveCompletion(currentLevelIndex); // クリア状態を保存
            
            showCustomModal(
                'CLEAR！', 
                `No.${currentRiddle.level} 「${currentRiddle.title}」を解き明かしました！`,
                '一覧に戻る',
                'bg-primary-gold hover:bg-secondary-sepia text-gray-900',
                () => showIndexPage()
            );
...
/**
 * 進捗状況をクリアするためのモーダルを表示します。
 * @param {string} type - 'main' (メイン謎) または 'mini' (プチ謎)
 */
function confirmClearProgress(type) {
    ...
    if (type === 'main') {
        ...
        title = 'メイン謎の進捗をリセット';
        content = '<p>謎コレクション（メイン謎）のクリア状況をすべてリセットします。よろしいですか？</p><p class="mt-2 text-sm text-red-400">※複数回答の進捗もリセットされます。</p>';
        onConfirm = () => {
            localStorage.removeItem(MAIN_STORAGE_KEY);
            localStorage.removeItem(MAIN_MULTI_STORAGE_KEY);
            showIndexPage(); // リストを再生成して更新
            showCustomModal('リセット完了', 'メイン謎の進捗をリセットしました。', 'OK', 'bg-green-600 hover:bg-green-700');
        };
    } else if (type === 'mini') {
        key = MINI_COMPLETED_ANSWERS_KEY;
        title = 'プチ謎の進捗をリセット';
        content = '<p>プチ謎チャレンジ（56問）の正解状況をすべてリセットします。よろしいですか？</p>';
        onConfirm = () => {
            localStorage.removeItem(MINI_COMPLETED_ANSWERS_KEY);
            showMiniChallengePage(); // 画面を再読み込みして更新
            showCustomModal('リセット完了', 'プチ謎の進捗をリセットしました。', 'OK', 'bg-green-600 hover:bg-green-700');
        };
    } else {
        return; // 無効なタイプ
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // URLのハッシュ（#introなど）をチェックして、対応する画面を表示するロジックがあれば、ここに追加します。
    // 現状はDOMContentLoaded時にshowIntroPageを呼び出しますが、HTMLにボタンがないため、
    // HTML側でbody要素などに`onload="showIntroPage()"`を設定するか、
    // ここで直接呼び出すためのHTML側修正が必要です。

    // HTML側の要素が存在するか確認し、存在すればイベントリスナーを設定する処理
    
    // 画面遷移関数はHTMLのボタンで直接呼び出す想定
    // e.g., <button onclick="showMainContent()">
    
    // ロード時にメインコンテンツを表示したい場合は以下をコメント解除
    // showMainContent();

});


// --- 10. グローバルスコープに公開する関数 (HTMLから直接呼び出す用) ---
// Note: DOMContentLoadedの外に出すことで、HTMLのonclickイベントからアクセスできるようにします。

window.showIntroPage = showIntroPage;
window.showMainContent = showMainContent;
window.switchCollection = switchCollection;
window.showIndexPage = showIndexPage;
window.loadRiddle = loadRiddle;
window.checkAnswer = checkAnswer;
window.hideModal = hideModal;

window.showMiniChallengePage = showMiniChallengePage;
window.showMiniIndexPage = showMiniIndexPage;
